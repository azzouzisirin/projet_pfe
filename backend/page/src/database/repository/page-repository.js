...
class ProductRepository {
    ...
    async CreateProduct({ userId, desc, img }){
     ...
     async pages(){
         try{
            ...
            throw APIError('API Error', STATUS_CODES.INTERNAL_ERROR, 'Unable to Get pages')
         }
    }
    ...
    async FindByCategory(category){
        ...
        try{
            const pages = await ProductModel.find({ type: category});
             return pages;
    ...
    async FindSelectedpages(selectedIds){
        try{
            const pages = await ProductModel.find().where('_id').in(selectedIds.map(_id => _id)).exec();
            return pages;
        }catch(err){
            throw APIError('API Error', STATUS_CODES.INTERNAL_ERROR, 'Unable to Find Product')
        }
